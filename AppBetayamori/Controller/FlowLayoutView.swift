...
struct FlowLayoutView: View {
    ...
    @State var whosTagArray : [TagDataModel]
    private let xSpace = 15
    private let ySpace = 0
    
//    init(){
//        getMusic(tagArray: whosTagArray)
//    }
    ...
    var body: some View {
        let music = getMusic(tagArray: whosTagArray)
        ScrollView {
            ...
            VStack(alignment: .leading){
                ...
                if music.count == 0 {
                    ...
                    FlexibleView(data: music){ item in
                        ...
                        Tag(tapState: false,color: model.tagCategoryList[item.parent] ?? .gray, text: item.name, size: .title3)
